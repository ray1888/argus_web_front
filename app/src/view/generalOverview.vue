<template>
  ...
  <div class="generalOverview">
    <div class="generalTop">
      ...
      <div class="searchBtn">
        <button class='clickable' type='button' @click="searchClick">搜索</button>
      </div>
      ...
      <table border="0" cellspacing="0" cellpadding="0">
        ...
        <tbody>
          <tr :key="item._id" v-for="(item,index) in List">
            ...
            <td v-if="power=='rw'" class="operation">
  ...
  export default {
    ...
    computed: {
      ...
      ...mapState({
        ...
        loginUser: state => state.login_module.userinfo.name
      })
    },
    watch: {
      pageactive: function(val) {
        //页号发生变化
        this.initInfo()
        this.$store.commit('setOverviewPar',{text: this.searchtext,tag: this.tagsle,page: this.pageactive});
      }
    },
    methods: {
      searchClick:function(){ //搜索按钮点击
        this.getViewcount()
        this.searchView();
        this.pageactive=1;
        this.$store.commit('setOverviewPar',{text: this.searchtext,tag: this.tagsle,page: this.pageactive});
      ...
      editView: function(item, index) {
        ...
        if (item) { //编辑
          ...
          this.view_data.name = ''
          this.view_data.tags = []
          this.view_data.creater = this.loginUser
        }
      ...
      getPar:function(){ //从vuex获取参数
          this.searchtext = this.overview.text;
          this.tagsle = this.overview.tag;
          this.pageactive = this.overview.page;
      }
    },
    beforeRouteLeave (to, from, next) { //不是去详情页就清空筛选条件
      if (to.path.indexOf('/navBar/monitorView') === -1) {
        this.$store.commit('setOverviewPar',{text: '',tag: '',page: 1,});
      }
      next()
    },
    mounted() {
