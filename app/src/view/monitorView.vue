<template>
  <div class="monitorView">
    ...
    <section class="top clearfix">
      ...
      <button v-if="power=='rw'" class="addChartBtn fr clickable" @click="editView()">添加监控图表</button>
      <button v-if="power=='rw'" class="lastTimeBtn fr clickable" @click="edit_time()">{{dateTimeString}}</button>
    </section>
    <!-- 顶部 -->
    <!-- 内容 -->
    <section class="main_content">
      <div class="content_bottom" v-for="(chart,index) in chartList" :key="index">
        <!-- 图表 -->
        <div class="tipInfo">
          <div>
            <h4 class="fl">{{chart.title}}</h4>
            <button v-if="power=='rw'" class="fr clickable" title="添加到概览" @click="addmychart(chart)"><i class="icon-pushToHome"></i></button>
            <button style="font-size: 0.8rem" class="fr clickable" title="刷新" @click="refreshClick(index)"><i class="icon-refresh" style="font-size: 0.8rem"></i></button>
            <!--<button class="fr clickable" title="切换折线图"><i class="icon-linechart"></i></button>-->
            <button v-if="power=='rw'" class="fr clickable" title="设置" @click="editView(chart)"><i class="icon-setting"></i></button>
          </div>
        </div>
        <div class="chart">
          <jkchart :option="chart" :date="dateTimeList[index]" :reloadhz="reloadhz"></jkchart>
        </div>
      </div>
    </section>
    <!-- 内容 -->
          ...
          <div class="main_box">
            ...
            <div class="baseInfo_box" v-show="monitoringInfo.navIndex === 0">
              ...
              <ul class="chart_type_box clearfix">
                <li class="fl clickable" v-if="chartVal=='折线图'||chartVal=='柱图'||chartVal=='散点图'" :class="{'active':editChart.nomalinfo.type===chartVal,'disable':chartVal!='折线图'&&chartVal!='柱图'&&chartVal!='散点图'}" @click="editChart.nomalinfo.type=chartVal" v-for="(chartVal,chartIndex) in monitoringInfo.chartType">{{chartVal}}</li>
                <li class="fl" v-if="chartVal!='折线图'&&chartVal!='柱图'&&chartVal!='散点图'" :class="{'active':editChart.nomalinfo.type===chartVal,'disable':chartVal!='折线图'&&chartVal!='柱图'&&chartVal!='散点图'}" v-for="(chartVal,chartIndex) in monitoringInfo.chartType">{{chartVal}}</li>
              </ul>
            ...
            <div class="target_box" v-show="monitoringInfo.navIndex === 1">
              <div class="target_content_box" v-for="(indexitem,index) in editChart.pfm_index">
                ...
                <button class="deletAllBtn btn clickable" @click="delxnzb(index)" type="button" title="删除"><i class="icon-delete"></i></button>
                <div class="one clearfix">
                  <p class="left_title fl">指标名</p>
                  <el-select v-model="indexitem.zbname" @change="zbnameChange(indexitem)" class="fl target_input ml" filterable remote reserve-keyword placeholder="请输入" :remote-method="getzbnameList" :loading="loading">
                ...
                <div class="two clearfix">
                  ...
                  <p class="right_title fl ml"><input class="text" v-model="indexitem.qytime" placeholder="时间" type="number" /></p>
                ...
                <ul>
                  <li class="clearfix" style="margin-bottom:0.25rem;" v-for="(tag,index) in indexitem.tag">
                    <el-select v-model="tag.key" @change="tagkeychoose(indexitem,tag)" class="fl key_input" placeholder="请选择">
                      <el-option @click="tag.value = '*';" v-for="(item,index) in indexitem.tagkeyList" :key="index" :label="item" :value="item">
                      </el-option>
                    </el-select>
                    ...
                    <button class="ml btn fl clickable" type="button" @click="deltag(indexitem.tag,index)" title="删除"><i class="icon-delete"></i></button>
                  </li>
                </ul>
              </div>
      ...
      <div class="time_edit_wrapper" v-show="editTimeShow" @click.self="editTimeShow=!editTimeShow">
        ...
        <div class="edit_time">
          <div class="btnBox selfTime">
            <!--快捷时间选择-->
            <button v-for="item in shelfTimes" @click="updatedatetimeconfigs('shelf',item.label,item.value);currentReloadIndex = {index:-1};" class="clickable" type="button">{{item.label}}</button>
          ...
          <div v-show="showdeftime" class="selfDefined">
            ...
            <div class="btnBox toolbuttons">
              <button @click="updatedatetimeconfigs('defined');currentReloadIndex = {index:-1};" class="clickable ok" type="button">确定</button>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        currentReloadIndex:{index:-1},//用于局部刷新
        loading: false, //是否在加载
    ...
    computed: {
      dateTimeList:function () { //每个图表的时间
        var arr = [];
        var _this = this;
        this.chartList.forEach(function () {
      ...
      power: function() {
        //权限
        return this.$store.state.login_module.userinfo.pri.jk
      },
    ...
    beforeRouteLeave(to, from, next) { //不是去详情页就清空筛选条件
      if (to.path.indexOf('/navBar/generalOverview') === -1) {
        this.$store.commit('setOverviewPar', {
          text: '',
          tag: '',
          page: 1,
        });
    ...
    methods: {
      datetimeFunc:function(index){
        this.currentReloadIndex = index;
        if(index !== this.currentReloadIndex){
          return this.datetime;
        }else{
          return {
            start:simdate.hour1_ago(),
            end:simdate.min5_ago()
          }
        }
      },
      ...
      updatedatetimeconfigs: function(type, label, value) {
        //更新视图数据的时间间隔配置
        return new Promise((resolve, reject) => {
          //时间选择类型
          if (type == 'shelf') {
            //快捷定义时间
            let dateTimeString = '最新' + label
            http.post(urls.GXSTSJPZ, {
              viewid: this.viewid, //视图id
              datatimeconfig: {
                viewid: this.viewid, //视图id
                start: value, //开始时间
                end: simdate.todayTime(), //结束时间
                datetext: dateTimeString, //时间描述文字
                reloadhz: this.definedTime.reloadhz //刷新频率
              }
            }).then(res => {
            ...
            //自定义时间选择
            let dateTimeString = '开始：' + simdate.TMDTime(this.definedTime.start) + ' 结束：' + simdate.TMDTime(this.definedTime.end)
            http.post(urls.GXSTSJPZ, {
              viewid: this.viewid, //视图id
              datatimeconfig: {
                viewid: this.viewid, //视图id
                start: this.definedTime.start, //开始时间
                end: this.definedTime.end, //结束时间
                datetext: dateTimeString, //时间描述文字
                reloadhz: this.definedTime.reloadhz //刷新频率
              }
            }).then(res => {
      ...
      editView: function(chart) {
        ...
        let chart_option = _.cloneDeep(chart)
        if (chart_option) {
          //编辑视图
          this.edittype = '编辑'
          this.editChart.nomalinfo._id = chart_option._id
          this.editChart.nomalinfo.title = chart_option.title
          this.editChart.nomalinfo.type = chart_option.type
          // this.editChart.pfm_index = chart_option.xnzbs
          this.editChart.ystyle = chart_option.yconfig
          // console.info('指标数组', this.editChart)
          let gettagvalues = [] //获取标签值请求数组
          chart_option.xnzbs.forEach((item) => {
            //遍历指标数组
      ...
      showdeftimeClick: function() { //自定义按钮点击事件
        if (this.showdeftime) {
          this.definedTime = Object.assign({}, this.datetime, {
            'reloadhz': this.reloadhz
          });
        }
      },
      ...
      addchart: function() {
        //添加图表
        let newoption = _.cloneDeep(this.editChart)  //新图表数据 （对象深拷贝）（发送前处理掉数据库不要的数据）
      ...
      refreshClick(index){ //刷新图表
        //this.datetime = Object.assign({},this.datetime);
        if (this.dateTimeString.indexOf('最新') != -1) {
          this.currentReloadIndex = {index:index};
        }
